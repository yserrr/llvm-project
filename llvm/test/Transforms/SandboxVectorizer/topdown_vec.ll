...
; CHECK-NEXT:    [[PTR2_0:%.*]] = getelementptr float, ptr [[PTR2]], i32 0
; CHECK-NEXT:    [[LD1:%.*]] = load float, ptr [[PTR1]], align 4, !sandboxvec [[META7:![0-9]+]]
; CHECK-NEXT:    [[PACK:%.*]] = insertelement <2 x float> poison, float [[X]], i32 0, !sandboxvec [[META7]]
; CHECK-NEXT:    [[PACK1:%.*]] = insertelement <2 x float> [[PACK]], float [[LD1]], i32 1, !sandboxvec [[META7]]
; CHECK-NEXT:    [[VECL:%.*]] = load <2 x float>, ptr [[PTR0]], align 4, !sandboxvec [[META7]]
; CHECK-NEXT:    [[VEC:%.*]] = fsub <2 x float> [[VECL]], [[PACK1]], !sandboxvec [[META7]]
; CHECK-NEXT:    store <2 x float> [[VEC]], ptr [[PTR2_0]], align 4, !sandboxvec [[META7]]
; CHECK-NEXT:    ret void
;
  %ptr0 = getelementptr float, ptr %ptr, i32 0
  %ptr1 = getelementptr float, ptr %ptr, i32 1
...
; VecUtils::getNextUserBundle() FIXME: lane 0's value is used twice in %fadd0
; but lane 1's matching user only uses %ld1 once (at operand 0). Picking the
; first operand index still forms {fadd0,fadd1} and incorrectly widens the
; fadds. Once fixed, the fadds should stay scalar and use unpacks:
;   fadd float [[UNPACK]], [[UNPACK]]
;   fadd float [[UNPACK1]], [[OTHER]]
...
; CHECK-NEXT:    [[VEC:%.*]] = fadd <2 x float> [[VECL]], [[VECL]], !sandboxvec [[META11]]
; CHECK-NEXT:    store <2 x float> [[VEC]], ptr [[PTR2_0]], align 4, !sandboxvec [[META11]]
; CHECK-NEXT:    ret void
;
  %ptr0 = getelementptr float, ptr %ptr, i32 0
  %ptr1 = getelementptr float, ptr %ptr, i32 1
