...
bool RegBankLegalizeHelper::lowerSetRounding(MachineInstr &MI) {
  ...
  if (auto ConstMode = getIConstantVRegValWithLookThrough(NewMode, MRI)) {
    ...
    if (UseReducedTable) {
      ...
      auto BitTable = B.buildConstant({SgprRB, LLT::integer(64)},
                                      AMDGPU::FltRoundToHWConversionTable);
...
bool RegBankLegalizeHelper::lowerGetRounding(MachineInstr &MI) {
  ...
  auto BitTable = B.buildConstant({SgprRB, LLT::integer(64)},
                                  AMDGPU::FltRoundConversionTable);
...
bool RegBankLegalizeHelper::applyMappingDst(
  ...
  for (; OpIdx < MethodIDs.size(); ++OpIdx) {
    ...
    case Sgpr32Trunc: {
      ...
      assert(RB == SgprRB);
      Register NewDst = MRI.createVirtualRegister({SgprRB, I32});
      Op.setReg(NewDst);
      if (!MRI.use_empty(Reg))
...
bool RegBankLegalizeHelper::applyMappingSrc(
  ...
  for (unsigned i = 0; i < MethodIDs.size(); ++OpIdx, ++i) {
    ...
    case Sgpr32AExt: {
      ...
      assert(Ty.getSizeInBits() < 32);
      assert(RB == SgprRB);
      auto Aext = B.buildAnyExt({SgprRB, I32}, Reg);
    ...
    case Sgpr32AExtBoolInReg: {
      ...
      auto Cst1 = B.buildConstant({SgprRB, I32}, 1);
      auto BoolInReg = B.buildAnd({SgprRB, I32}, Aext, Cst1);
    ...
    case Sgpr32SExt: {
      ...
      assert(RB == SgprRB);
      auto Sext = B.buildSExt({SgprRB, I32}, Reg);
    ...
    case Sgpr32ZExt: {
      assert(1 < Ty.getSizeInBits() && Ty.getSizeInBits() < 32);
      assert(RB == SgprRB);
      auto Zext = B.buildZExt({SgprRB, I32}, Reg);
    ...
    case Vgpr32AExt: {
      assert(Ty.getSizeInBits() < 32);
      assert(RB == VgprRB);
      auto Aext = B.buildAnyExt({VgprRB, I32}, Reg);
      Op.setReg(Aext.getReg(0));
      break;
    }
    case Vgpr32SExt: {
      ...
      assert(Ty.getSizeInBits() < 32);
      assert(RB == VgprRB);
      auto Sext = B.buildSExt({VgprRB, I32}, Reg);
      Op.setReg(Sext.getReg(0));
      break;
    }
    case Vgpr32ZExt: {
      ...
      assert(Ty.getSizeInBits() < 32);
      assert(RB == VgprRB);
      auto Zext = B.buildZExt({VgprRB, I32}, Reg);
      Op.setReg(Zext.getReg(0));
      break;
    }
    default:
