...
markMetadirectiveLoopIVs(semantics::SemanticsContext &semaCtx,
  ...
  for (int64_t level = 0; level < affectedDepth; ++level) {
    ...
    // (check-omp-variant.cpp), so it never reaches lowering in a well-formed
    // program.
    assert((doConstruct->IsDoNormal() || doConstruct->IsDoConcurrent()) &&
           "metadirective affected loop must be a canonical DO loop");
